...
def get_score(solution: GameRecord, answers: dict[str, GameRecord]) -> dict[str, float]:
    """State-of-the art, sophisticated scoring algorithm.
    ...
    for idx, ans in solution.items():
        for p in answers:
            _current_answer = answers[p][idx]
            _LOGGER.debug(f"{p=} q={idx} a={ans} {_current_answer=}")
            if idx == 1:
                if not isinstance(_current_answer, float) and not isinstance(
                    ans, float
                ):
                    raise ValueError("IDX 2 should be float")
                _diff = abs(float(ans) - float(_current_answer))
                if _diff < 3:
                    scores[p] += 1
                elif _diff < 6:
                    scores[p] += 0.5
                ...
                isinstance(ans, str)
                and isinstance(_current_answer, str)
                and ans.lower() == _current_answer.lower()
            ):
                scores[p] += 1
...
def color_value(v: float) -> Text:
    _scale = 5
    color, _ = get_color(_scale * int(v))
    return Text(str(v), Style(color=color, bgcolor=None))


def build_score_table(scores: dict[str, float]):
...
def main(solution: Path, answer: Iterable[Path], debug: bool):
    ...
    table = Align.center(build_score_table(scr), vertical="middle")
    console = Console()
    console.print(table)
